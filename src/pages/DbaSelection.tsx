...
const DbaSelection: React.FC = () => {
  ...
  useEffect(() => {
    const loadDbaSelections = async (retryCount = 0) => {
      try {
        console.log(
          '[DBA SELECTION] Loading DBA selections...',
          retryCount > 0 ? `(retry ${retryCount})` : ''
        );
        const selections = await dbaSelectionApi.getDbaSelections({ active: true }); // Only active selections
        setDbaSelections(selections || []); // Ensure we always have an array
  ...
  useEffect(() => {
    if (loadingDbaSelections || loading) {
      return;
    } // Wait for both collection and DBA selections to load
  ...
  const handleMarkForDba = async () => {
    if (selectedItems.length === 0) {
      handleApiError(
        new Error('Please select at least one item to mark for DBA'),
        'No items selected'
      );
      return;
    }
    ...
    try {
      ...
      setAllItems(prev =>
        prev.map(item => {
          const dbaInfo = updatedSelections.find(
            s => s.itemId === item.id && s.itemType === item.type
          );
          if (dbaInfo) {
            return {
              ...item,
  ...
  const handleRemoveFromDba = async () => {
    if (selectedItems.length === 0) {
      handleApiError(
        new Error('Please select at least one item to remove from DBA'),
        'No items selected'
      );
      return;
    }
        ...
        <div className='space-y-3'>
          ...
          {dbaInfo.selectedForDba ? (
            ...
            <div className='bg-slate-50 rounded-lg p-3 border border-slate-200'>
              <div className='text-sm text-slate-600 text-center'>Not selected for DBA</div>
            </div>
          )}
        </div>
      </div>
  ...
  const headerActions = (
    <div className='flex items-center space-x-3'>
      ...
      <div className='text-right'>
        <div className='text-2xl font-bold text-slate-900'>{selectedForDbaItems.length}</div>
        <div className='text-sm text-slate-600 font-medium'>Items Selected</div>
      </div>
    </div>
  );
  ...
  return (
    <PageLayout
      title='DBA Selection'
      subtitle='Manage items selected for DBA (Den Blå Avis) listing'
      loading={loading}
      error={error}
          ...
          {allItems.length === 0 && (
            <div className='bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-16 text-center'>
              ...
              <p className='text-slate-600'>
                Add some items to your collection first to manage DBA selections
              </p>
            </div>
          )}
        </div>
