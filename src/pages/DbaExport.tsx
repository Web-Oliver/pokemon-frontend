...
const DbaExport: React.FC = () => {
  ...
  useEffect(() => {
    const loadDbaSelections = async (retryCount = 0) => {
      try {
        console.log('[DBA EXPORT] Loading DBA selections...', retryCount > 0 ? `(retry ${retryCount})` : '');
        ...
        console.error('[DBA EXPORT] Failed to load DBA selections:', error);
        ...
        // Don't fail the page, just set empty array
        setDbaSelections([]);
      } finally {
  ...
  const getDbaInfo = (itemId: string, itemType: string) => {
    return dbaSelections.find(selection => 
  ...
  const updateItemCustomization = (itemId: string, field: 'customTitle' | 'customDescription', value: string) => {
  ...
  const generateDefaultDescription = (item: SelectedItem & any): string => {
    const baseDesc = customDescription.trim() || 'Kan afhentes i København eller sendes med GLS, Jeg er ikke interesseret i at bytte.';
  ...
  const handleExportToDba = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log('[DBA EXPORT] Successfully reloaded DBA selections with', updatedSelections.length, 'items');
      } catch (dbaError) {
      ...
      showSuccessToast(`DBA export generated successfully! ${response.data.itemCount} items exported and added to DBA tracking.`);
      
    } catch (error) {
      console.error('[DBA EXPORT] Export failed:', error);
      handleApiError(error, 'Failed to export to DBA format');
    } finally {
  ...
  const handleDownloadZip = async () => {
    try {
      ...
      handleApiError(error, 'Failed to download DBA export');
    } finally {
      setIsExporting(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto space-y-10">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">{dbaSelections.length}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Expiring Soon</p>
                  <p className="text-2xl font-bold text-red-600">{dbaSelections.filter(s => s.daysRemaining <= 10).length}</p>
                </div>
                <div>
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center flex-1">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500 capitalize">{item.type} • {item.price} DKK</p>
                            </div>
                          </div>
                          <Button
                            onClick={() => {
                              if (editingItem === item.id) {
                                ...
                                if (!item.customTitle) {
                                  updateItemCustomization(item.id, 'customTitle', generateDefaultTitle(item));
                                }
                                if (!item.customDescription) {
                                  updateItemCustomization(item.id, 'customDescription', generateDefaultDescription(item));
                                }
                          ...
                          <div className="space-y-3 pt-3 border-t border-slate-200">
                            <div>
                              ...
                              <div className="flex justify-between items-center mt-1">
                                ...
                                <span className={`text-xs ${
                                  getCurrentTitle(item).length > 70 ? 'text-red-500' : 'text-slate-400'
                                }`}>
                            ...
                            <div>
                              ...
                              <textarea
                                ...
                                onChange={(e) => updateItemCustomization(item.id, 'customDescription', e.target.value)}
                ...
                {exportResult && (
                  <div className="mt-4 p-4 bg-green-50 rounded-xl border border-green-200">
                    ...
                    <p className="text-sm text-green-700">
                      Generated {exportResult.itemCount} DBA posts. Files saved to: {exportResult.dataFolder}
                    </p>
                  </div>
                )}
          ...
          <div className="space-y-8">
            ...
            {(() => {
              ...
              const sealedWithTimers = sealedProducts.filter(product => getDbaInfo(product.id || product._id, 'sealed'));
              
              const totalWithTimers = psaWithTimers.length + rawWithTimers.length + sealedWithTimers.length;
              
              if (totalWithTimers === 0) return null;
            ...
            {(() => {
              ...
              const psaWithoutTimers = psaCards.filter(card => !getDbaInfo(card.id || card._id, 'psa'));
              const rawWithoutTimers = rawCards.filter(card => !getDbaInfo(card.id || card._id, 'raw'));
              const sealedWithoutTimers = sealedProducts.filter(product => !getDbaInfo(product.id || product._id, 'sealed'));
              
              const totalWithoutTimers = psaWithoutTimers.length + rawWithoutTimers.length + sealedWithoutTimers.length;
              
              if (totalWithoutTimers === 0) return null;
          ...
          {psaCards.length === 0 && rawCards.length === 0 && sealedProducts.length === 0 && (
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-16 text-center">
              ...
              <p className="text-slate-600">Add some items to your collection first to export them to DBA.dk</p>
            </div>
          )}
        </div>
      </div>
