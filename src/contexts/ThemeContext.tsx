...
export type VisualTheme = 'context7-premium' | 'context7-futuristic' | 'dba-cosmic' | 'minimal';
...
export const themePresets: ThemePreset[] = [
  {
    ...
    description: 'Glassmorphism design with premium gradients and micro-interactions',
    config: {
  ...
  },
  {
    id: 'dba-cosmic',
    name: 'DBA Cosmic',
    description: 'Space-themed with cosmic gradients and holographic backgrounds',
    config: {
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateCSSProperties = useCallback((themeConfig: ThemeConfiguration) => {
    ...
    root.style.setProperty('--glass-blur', `${themeConfig.glassmorphismIntensity / 5}px`);
  ...
  const setGlassmorphismIntensity = useCallback((intensity: number) => {
    setConfig(prev => ({ ...prev, glassmorphismIntensity: Math.max(0, Math.min(100, intensity)) }));
  }, []);

  const toggleParticleEffects = useCallback(() => {
    setConfig(prev => ({ ...prev, particleEffectsEnabled: !prev.particleEffectsEnabled }));
  }, []);

  const setCustomProperties = useCallback((properties: Record<string, string>) => {
  ...
  const saveCustomPreset = useCallback((name: string, themeConfig: ThemeConfiguration) => {
    try {
      const customPresets = JSON.parse(localStorage.getItem('pokemon-custom-presets') || '{}');
      customPresets[name] = themeConfig;
      localStorage.setItem('pokemon-custom-presets', JSON.stringify(customPresets));
    } catch (error) {
  ...
  const getThemeClasses = useCallback((): string => {
    ...
    if (!config.particleEffectsEnabled) classes.push('no-particles');
