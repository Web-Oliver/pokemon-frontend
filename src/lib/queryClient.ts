  ...
  defaultOptions: {
    queries: {
      ...
      refetchOnWindowFocus: (query) => {
        ...
        const isStaticData = query.queryKey.includes('cards-reference') || 
      ...
      refetchOnReconnect: (query) => {
        const isCriticalData = query.queryKey.includes('collection') ||
...
export const queryKeys = {
  ...
  psaCards: (filters?: any) => [...queryKeys.collection, 'psa-cards', filters] as const,
  rawCards: (filters?: any) => [...queryKeys.collection, 'raw-cards', filters] as const,
  sealedProducts: (filters?: any) => [...queryKeys.collection, 'sealed-products', filters] as const,
  soldItems: (filters?: any) => [...queryKeys.collection, 'sold-items', filters] as const,
  ...
  itemHistory: (type: string, id: string) => ['item-history', type, id] as const,
  itemImages: (type: string, id: string) => ['item-images', type, id] as const,

  // === REFERENCE DATA (Static - Long Cache) ===
  reference: ['reference'] as const,
  cardsReference: (params?: any) => [...queryKeys.reference, 'cards', params] as const,
  setsReference: (params?: any) => [...queryKeys.reference, 'sets', params] as const,
  productsReference: (params?: any) => [...queryKeys.reference, 'products', params] as const,
  gradesReference: () => [...queryKeys.reference, 'grades'] as const,
  conditionsReference: () => [...queryKeys.reference, 'conditions'] as const,

  // === SEARCH DATA ===
  search: ['search'] as const,
  searchCards: (query: string, filters?: any) => [...queryKeys.search, 'cards', { query, filters }] as const,
  searchSets: (query: string, filters?: any) => [...queryKeys.search, 'sets', { query, filters }] as const,
  searchProducts: (query: string, filters?: any) => [...queryKeys.search, 'products', { query, filters }] as const,
  searchHistory: () => [...queryKeys.search, 'history'] as const,
  searchSuggestions: (type: string, query: string) => [...queryKeys.search, 'suggestions', type, query] as const,

  // === ANALYTICS DATA ===
  analytics: ['analytics'] as const,
  salesAnalytics: (dateRange?: any) => [...queryKeys.analytics, 'sales', dateRange] as const,
  valueAnalytics: (dateRange?: any) => [...queryKeys.analytics, 'value', dateRange] as const,
  gradeDistribution: () => [...queryKeys.analytics, 'grade-distribution'] as const,
  setDistribution: () => [...queryKeys.analytics, 'set-distribution'] as const,
  profitAnalytics: (dateRange?: any) => [...queryKeys.analytics, 'profit', dateRange] as const,

  // === ACTIVITY & AUDIT ===
  activity: ['activity'] as const,
  activityFeed: (filters?: any) => [...queryKeys.activity, 'feed', filters] as const,
  recentActivity: () => [...queryKeys.activity, 'recent'] as const,
  auditLog: (filters?: any) => [...queryKeys.activity, 'audit', filters] as const,

  // === AUCTIONS ===
  auctions: ['auctions'] as const,
  auctionsList: (filters?: any) => [...queryKeys.auctions, 'list', filters] as const,
  ...
  dbaExport: (params?: any) => [...queryKeys.dbaSelection, 'export', params] as const,
...
export const cacheInvalidation = {
  ...
  onUpdateItem: (itemType: string, itemId: string) => {
    ...
    queryClient.invalidateQueries({ queryKey: queryKeys.collectionItem(itemType, itemId) });
  ...
  onDeleteItem: (itemType: string, itemId: string) => {
    ...
    queryClient.removeQueries({ queryKey: queryKeys.collectionItem(itemType, itemId) });
    queryClient.removeQueries({ queryKey: queryKeys.itemHistory(itemType, itemId) });
    queryClient.removeQueries({ queryKey: queryKeys.itemImages(itemType, itemId) });
  ...
  onUpdateAuction: (auctionId: string) => {
    ...
    queryClient.invalidateQueries({ queryKey: queryKeys.auctionItems(auctionId) });
  ...
  // Full cache reset (for debugging or major updates)
  resetAllCache: () => {
    queryClient.clear();
  },
  ...
  warmupCache: async () => {
    // Prefetch critical collection data
    ...
    await queryClient.prefetchQuery({
      queryKey: queryKeys.setsReference(),
      staleTime: CACHE_TIMES.STATIC_DATA.staleTime,
    });
  },
};

// Cache utilities for monitoring and management
export const cacheUtils = {
  ...
  getCacheStats: () => {
    ...
    return {
      ...
      activeQueries: queries.filter(query => query.observers.length > 0).length,
      cachedQueries: queries.filter(query => query.state.data !== undefined).length,
      errorQueries: queries.filter(query => query.state.error !== null).length,
  ...
  cleanupCache: () => {
    ...
    // Remove queries that haven't been used in 1 hour and have no observers
    queries.forEach(query => {
      const lastUsed = query.state.dataUpdatedAt;
      const oneHourAgo = Date.now() - (1000 * 60 * 60);
      
      if (lastUsed < oneHourAgo && query.observers.length === 0) {
        cache.remove(query);
      }
    });
    
    log('[CACHE CLEANUP] Removed unused queries', {
      beforeCount: queries.length,
      afterCount: cache.getAll().length,
    });
  },
