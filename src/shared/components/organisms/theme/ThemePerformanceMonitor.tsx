...
export const ThemePerformanceMonitor: React.FC<ThemePerformanceMonitorProps> = ({
  ...
  const collectPerformanceData = (): PerformanceData => {
    ...
    const themeEntries = performance.getEntriesByType('measure')
      .filter(entry => entry.name.includes('theme'));
    const themeLoadTime = themeEntries.length > 0 
  ...
  const averageRenderTime = performanceHistory.length > 0
    ? performanceHistory.reduce((sum, data) => sum + data.renderTime, 0) / performanceHistory.length
    : 0;

  const memoryTrend = performanceHistory.length > 1
    ? performanceHistory[performanceHistory.length - 1].memoryUsage - performanceHistory[0].memoryUsage
    : 0;
    ...
    <div className="bg-zinc-900/50 rounded-lg p-4 border border-zinc-700/50">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-zinc-800/50 rounded-lg p-3">
          ...
          <div className="text-xs text-zinc-500">
            {memoryTrend > 0 ? '+' : ''}{memoryTrend.toFixed(1)}MB
          </div>
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-zinc-700/50">
        <div className="flex items-center gap-2">
          <div className={cn(
            ...
            performanceData.renderTime < 16 && performanceData.memoryUsage < 100 
              ? "bg-green-400" 
              : performanceData.renderTime < 32 && performanceData.memoryUsage < 200
              ? "bg-yellow-400"
              : "bg-red-400"
          )} />
