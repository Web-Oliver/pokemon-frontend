...
export const useFormSubmission = <TFormData = any, TSubmissionData = any>(
   ...
   */
  const withSubmissionHandling = useCallback(
    async (operation: () => Promise<any>, operationName = 'operation') => {
      try {
        if (debug) {
        ...
        if (debug) {
          console.log(
            `[${logContext}] ${operationName} completed successfully`
          );
        }
  ...
  const handleSubmission = useCallback(
    async (
      ...
      try {
        ...
        if (debug) {
          console.log(
            `[${logContext}] Uploaded ${imageUrls.length} images:`,
            imageUrls
          );
        }
        ...
        if (debug) {
          console.log(
            `[${logContext}] Prepared submission data:`,
            submissionData
          );
        }
