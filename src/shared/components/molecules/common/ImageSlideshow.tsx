...
export const ImageSlideshow: React.FC<ImageSlideshowProps> = memo(
  ({
    ...
    if (!hasImages) {
      return (
        <div
          className={`w-full h-48 bg-zinc-800/60 rounded-xl flex items-center justify-center shadow-2xl bg-background border-border ${className}`}
          style={{
            boxShadow:
      ...
      <div className="w-full h-full">
        ...
        <div className="relative w-full h-full group">
          ...
          {hasMultipleImages && (
            <>
              <button
                className={`absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-zinc-900/80 hover:bg-zinc-800/90 text-zinc-300 hover:text-white rounded-lg sm:rounded-xl flex items-center justify-center shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-110 active:scale-95 z-50 bg-background border-border hover:border-zinc-600 backdrop-blur-xl group`}
                onClick={scrollPrev}
                      ...
                      <button
                        ...
                        className={`relative z-50 w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-xl sm:rounded-2xl overflow-hidden border-2 transition-all duration-300 group cursor-pointer ${
                          isActive
                            ...
                            : `bg-background border-border hover:border-zinc-600/70 shadow-lg hover:shadow-xl hover:scale-105 active:scale-95`
                        }`}
                        type="button"
