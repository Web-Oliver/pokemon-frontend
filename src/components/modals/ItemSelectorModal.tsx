...
export function ItemSelectorModal<T extends SelectableItem>({
  ...
  const filteredItems = useMemo(() => {
    ...
    if (filterCategory !== 'all') {
      if (categoryFilter) {
        filtered = filtered.filter(item => categoryFilter(item, filterCategory));
      } else {
    ...
    return filtered;
  }, [items, excludedSet, searchTerm, filterCategory, searchFilter, categoryFilter]);
  ...
  const handleSelectAll = () => {
    ...
    if (selectedItems.size === filteredItems.length) {
      ...
      const itemsToSelect = maxSelection ? filteredItems.slice(0, maxSelection) : filteredItems;
