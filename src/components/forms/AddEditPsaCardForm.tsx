...
const AddEditPsaCardForm: React.FC<AddEditPsaCardFormProps> = ({
  ...
  useEffect(() => {
    const handleCardAutoFill = async () => {
      if (cardProductName && activeField === null) {
        try {
          const bestMatch = await getBestMatch(cardProductName);
          if (bestMatch) {
            setValue('pokemonNumber', bestMatch.pokemonNumber || '');
            setValue('baseName', bestMatch.baseName || '');
            setValue('variety', bestMatch.variety || '');
            // Note: myPrice and grade remain user-editable
          }
        } catch (error) {
          console.error('Failed to auto-fill card information:', error);
        }
      }
    };
  ...
  const handleSuggestionClick = (suggestion: string, fieldType: 'set' | 'cardProduct') => {
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="relative">
            ...
            {/* Set Suggestions Dropdown */}
            {showSuggestions && activeField === 'set' && suggestions.length > 0 && (
              <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                {suggestions.map((suggestion, index) => (
                  <button
                    key={index}
                    type="button"
                    onClick={() => handleSuggestionClick(suggestion, 'set')}
          ...
          <div className="relative">
            ...
            {showSuggestions && activeField === 'cardProduct' && suggestions.length > 0 && (
              <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                {suggestions.map((suggestion, index) => (
                  <button
                    key={index}
                    type="button"
                    onClick={() => handleSuggestionClick(suggestion, 'cardProduct')}
                    className="w-full px-3 py-2 text-left hover:bg-blue-50 focus:bg-blue-50 focus:outline-none"
                  >
                    <span className="text-sm text-gray-900">{suggestion}</span>
                  </button>
