...
interface SearchDropdownProps {
  ...
  onSuggestionSelect: (suggestion: SearchSuggestion, fieldType: 'set' | 'category' | 'cardProduct') => void;
  onClose: () => void;
  searchTerm: string;
}
...
const SearchDropdown: React.FC<SearchDropdownProps> = ({
  ...
  const renderSuggestionMetadata = (suggestion: SearchSuggestion) => {
    ...
    if (suggestion.setInfo?.setName) {
      metadata.push(
        <div key="set" className="inline-flex items-center px-2 py-1 bg-gradient-to-r from-emerald-100 to-green-100 border border-emerald-200 rounded-full">
          <Package className="w-3 h-3 text-emerald-600 mr-1" />
          <span className="text-xs font-semibold text-emerald-800">{suggestion.setInfo.setName}</span>
        </div>
      );
    }
    ...
    if (suggestion.categoryInfo?.category) {
      metadata.push(
        <div key="category" className="inline-flex items-center px-2 py-1 bg-gradient-to-r from-purple-100 to-indigo-100 border border-purple-200 rounded-full">
          <Hash className="w-3 h-3 text-purple-600 mr-1" />
          <span className="text-xs font-semibold text-purple-800">{suggestion.categoryInfo.category}</span>
        </div>
      );
    }
    ...
    if (suggestion.variety) {
      metadata.push(
        <div key="variety" className="inline-flex items-center px-2 py-1 bg-gradient-to-r from-yellow-100 to-orange-100 border border-yellow-200 rounded-full">
    ...
    if (suggestion.year) {
      metadata.push(
        <div key="year" className="inline-flex items-center px-2 py-1 bg-gradient-to-r from-gray-100 to-slate-100 border border-gray-200 rounded-full">
    ...
    if (suggestion.counts) {
      ...
      metadata.push(
        <div key="counts" className="inline-flex items-center px-2 py-1 bg-gradient-to-r from-blue-100 to-cyan-100 border border-blue-200 rounded-full">
    ...
    if (suggestion.psaTotalGradedForCard && suggestion.psaTotalGradedForCard > 0) {
      metadata.push(
        <div key="psa" className="inline-flex items-center px-2 py-1 bg-gradient-to-r from-purple-100 to-pink-100 border border-purple-200 rounded-full">
          <TrendingUp className="w-3 h-3 text-purple-600 mr-1" />
          <span className="text-xs font-semibold text-purple-800">{suggestion.psaTotalGradedForCard.toLocaleString()} PSA</span>
        </div>
      );
    }
  ...
  const getDisplayName = (suggestion: SearchSuggestion) => {
    return suggestion.cardName || suggestion.name || suggestion.setName || suggestion.category || 'Unknown';
  };
  ...
  const highlightSearchTerm = (text: string, term: string) => {
    ...
    return parts.map((part, index) => 
      regex.test(part) ? (
        <span key={index} className="bg-gradient-to-r from-yellow-200 to-yellow-300 text-yellow-900 px-1 rounded font-bold">
          {part}
        </span>
      ) : part
    );
