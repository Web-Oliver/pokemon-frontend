 ...
 */
const SortableCategoryOrderingListComponent: React.FC<SortableCategoryOrderingListProps> = ({
  items,
  itemOrder,
  selectedItemIds,
  onReorderItems,
  ...
  const categorizedItems = useMemo(() => {
    ...
    const orderedItems = itemOrder.length > 0
      ? itemOrder.map(id => items.find(item => item.id === id)).filter(Boolean) as CollectionItem[]
      : items;
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (!dragConstraints.allowCrossCategoryDrag && activeCategory !== overCategory) {
      return; // Don't allow cross-category dragging
    }

    // Check selection constraints
    if (dragConstraints.restrictToSelectedItems) {
      if (!selectedItemIds.includes(activeItemId) || !selectedItemIds.includes(overItemId)) {
          ...
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2 hover:bg-zinc-800 p-2 rounded-lg transition-colors">
              <categoryConfig.icon className={`w-5 h-5 ${
                categoryConfig.color === 'teal' ? 'text-teal-400' :
          ...
          <div className="flex items-center space-x-2">
            <select 
              className={`text-xs px-2 py-1 rounded-lg font-medium ${
                categoryConfig.color === 'teal' ? 'bg-teal-900/30 border border-teal-600/40 text-teal-300' :
                categoryConfig.color === 'blue' ? 'bg-blue-900/30 border border-blue-600/40 text-blue-300' :
            ...
            {categoryItems.map((item, index) => (
              <SortableItemCard
                ...
                className={`border-2 transition-all duration-200 cursor-move hover:shadow-md ${
                  categoryConfig.color === 'teal' ? 'border-teal-200 hover:border-teal-300' :
                  categoryConfig.color === 'blue' ? 'border-blue-200 hover:border-blue-300' :
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-blue-300">
                  {items.length} items across {Object.values(categorizedItems).filter(items => items.length > 0).length} categories
                </p>
              </div>
              <div className="text-right">
                <p className="text-sm text-blue-300">
                  {dragConstraints.allowCrossCategoryDrag ? 'Cross-category' : 'Within-category'} dragging enabled
                </p>
