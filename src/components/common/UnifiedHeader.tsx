...
const UnifiedHeader: React.FC<UnifiedHeaderProps> = ({
  ...
  const variantConfigs = {
    glassmorphism: {
      ...
      effects: (
        <>
          ...
          <div className="absolute top-8 right-8 w-20 h-20 border-2 border-cyan-400/50 rounded-2xl rotate-45 animate-spin opacity-40 shadow-[0_0_20px_rgba(6,182,212,0.3)]" style={{animationDuration: '20s'}} />
      ...
      titleClasses: 'font-black tracking-tight bg-gradient-to-r from-cyan-300 via-purple-300 to-pink-300 bg-clip-text text-transparent drop-shadow-[0_0_30px_rgba(6,182,212,0.5)]',
      subtitleClasses: 'text-cyan-100/90 font-medium leading-relaxed',
      iconContainer: 'bg-gradient-to-br from-cyan-500/20 via-purple-500/20 to-pink-500/20 backdrop-blur-sm rounded-[1.5rem] shadow-2xl border border-white/[0.15] group-hover:scale-105 transition-all duration-500',
    ...
    cosmic: {
      ...
      titleClasses: 'font-bold tracking-wide bg-gradient-to-r from-purple-300 to-cyan-300 bg-clip-text text-transparent',
      subtitleClasses: 'text-purple-200/80',
      iconContainer: 'bg-gradient-to-br from-purple-600/30 to-cyan-600/30 rounded-2xl shadow-lg border border-purple-400/30',
    ...
    minimal: {
      ...
      iconContainer: 'bg-zinc-100 dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700',
  ...
  const StatItem: React.FC<{ stat: HeaderStat }> = ({ stat }) => {
    ...
    return (
      <div className="text-center">
        <StatIcon className={`w-4 h-4 mx-auto mb-1 ${variantClasses[stat.variant || 'default']}`} />
  ...
  return (
    <div className={`${variantConfig.container.join(' ')} ${sizeConfig.container} ${className}`}>
      {/* Background effects */}
      {variantConfig.effects}
      
      <div className="relative z-10">
        {/* Main content */}
        <div className={`flex items-start ${centered ? 'justify-center text-center' : 'justify-between'} mb-6`}>
          <div className="flex items-center flex-1">
            ...
            {/* Icon */}
            {Icon && (
              <div className={`${variantConfig.iconContainer} ${sizeConfig.iconContainer} flex items-center justify-center mr-6 relative`}>
            ...
            <div className="flex-1">
              <h1 className={`${variantConfig.titleClasses} ${sizeConfig.title} mb-2`}>
                {title}
              </h1>
              {subtitle && (
                <p className={`${variantConfig.subtitleClasses} ${sizeConfig.subtitle} flex items-center gap-3`}>
                  {variant === 'glassmorphism' && <span className="w-5 h-5 text-cyan-400 animate-pulse">✨</span>}
                  {subtitle}
                </p>
              )}
            </div>
        ...
        {/* Stats grid */}
        {stats.length > 0 && (
          <div className={`grid gap-4 mb-6 ${stats.length <= 2 ? 'grid-cols-2' : stats.length <= 4 ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-3 md:grid-cols-5'}`}>
            {stats.map((stat, index) => (
              <div key={index} className="bg-zinc-700/50 backdrop-blur-sm rounded-lg p-3">
                <StatItem stat={stat} />
              </div>
            ))}
          </div>
