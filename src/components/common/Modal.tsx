...
import { cn, cardStyleConfig, generateThemeClasses } from '../../utils/themeUtils';
...
const Modal: React.FC<ModalProps> = ({
  ...
  const maxWidthClass = sizeClasses[effectiveSize as keyof typeof sizeClasses] || sizeClasses.md;
  ...
  const modalPanelClasses = cn(
    ...
    // Size classes
    maxWidthClass,
    effectiveSize === 'fullscreen' ? 'w-full h-full rounded-none' : 'rounded-3xl',
    
    // Theme-aware card styling
    cardClasses({
      theme: effectiveTheme,
      size: effectiveDensity === 'compact' ? 'sm' : effectiveDensity === 'spacious' ? 'lg' : 'md',
      interactive: false,
  ...
  const closeButtonClasses = cn(
    ...
    effectiveAnimationIntensity === 'enhanced' && 'hover:shadow-lg hover:scale-105 active:scale-95'
  );
        ...
        {particleEffectsEnabled && effectiveTheme !== 'minimal' && (
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className={cn(
              ...
              effectiveTheme === 'dba-cosmic' ? 'bg-purple-400/20' : 'bg-zinc-400/20'
            )}></div>
            <div className={cn(
              'absolute top-3/4 right-1/3 w-1 h-1 rounded-full animate-bounce delay-100',
              effectiveTheme === 'context7-futuristic' ? 'bg-cyan-300/30' : 'bg-indigo-300/30'
            )}></div>
            <div className={cn(
              'absolute bottom-1/4 left-1/3 w-1.5 h-1.5 rounded-full animate-pulse delay-200',
              effectiveTheme === 'dba-cosmic' ? 'bg-pink-300/25' : 'bg-purple-300/25'
            )}></div>
          </div>
        )}
        ...
        <div className={modalPanelClasses}>
          ...
          <div className={cn(
            ...
            effectiveTheme === 'dba-cosmic' 
              ...
              : effectiveTheme !== 'minimal' && 'bg-gradient-to-r from-theme-primary/20 via-theme-accent/20 to-theme-primary/20'
          )} />
                ...
                {showCloseButton && (
                  ...
                  >
                    <X className={cn(
                      ...
                      effectiveAnimationIntensity === 'enhanced' && 'group-hover:rotate-90'
                    )} />
                  </button>
                )}
              </div>
          ...
          {effectiveAnimationIntensity !== 'disabled' && effectiveTheme !== 'minimal' && (
            <div className={cn(
          ...
          {particleEffectsEnabled && effectiveTheme !== 'minimal' && (
            <>
              <div className={cn(
                ...
                effectiveTheme === 'dba-cosmic' ? 'bg-purple-400' : 'bg-cyan-400'
              )}></div>
              <div className={cn(
                'absolute bottom-8 left-8 w-0.5 h-0.5 rounded-full animate-pulse opacity-40 delay-500',
                effectiveTheme === 'dba-cosmic' ? 'bg-pink-400' : 'bg-blue-400'
              )}></div>
            </>
          )}
        </div>
