...
export const PokemonIcon: React.FC<PokemonIconProps> = ({
  ...
  const { orbital = false, pulsing = false, glowing = false, floating = false } = effects;

  // Base foundation - used by ALL icon containers
  const baseClasses = [
    'relative flex items-center justify-center',
    'transition-all duration-500 group',
    interactive && 'cursor-pointer hover:scale-110 hover:rotate-6 active:scale-95'
  ].filter(Boolean).join(' ');
  ...
  const getStyleClasses = (variant: string, style: string) => {
    const variants = {
      primary: {
        ...
        glass: 'bg-cyan-500/10 backdrop-blur-lg border border-cyan-400/30 text-cyan-200'
      },
      secondary: {
        neural: [
        ...
        glass: 'bg-purple-500/10 backdrop-blur-lg border border-purple-400/30 text-purple-200'
      },
      success: {
        neural: [
        ...
        outline: 'border-2 border-emerald-400/50 text-emerald-400 bg-transparent',
        glass: 'bg-emerald-500/10 backdrop-blur-lg border border-emerald-400/30 text-emerald-200'
      },
      warning: {
        neural: [
        ...
        glass: 'bg-amber-500/10 backdrop-blur-lg border border-amber-400/30 text-amber-200'
      },
      danger: {
        neural: [
        ...
        glass: 'bg-red-500/10 backdrop-blur-lg border border-red-400/30 text-red-200'
      },
      info: {
        neural: [
        ...
        glass: 'bg-blue-500/10 backdrop-blur-lg border border-blue-400/30 text-blue-200'
      },
      neutral: {
        neural: [
        ...
        glass: 'bg-zinc-500/10 backdrop-blur-lg border border-zinc-400/30 text-zinc-200'
      },
      gradient: {
        neural: [
        ...
        outline: 'border-2 border-purple-400/50 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-pink-400',
        glass: 'bg-gradient-to-br from-cyan-500/10 via-purple-500/10 to-pink-500/10 backdrop-blur-lg border border-purple-400/30 text-white'
      }
    };
    
    return variants[variant as keyof typeof variants]?.[style as keyof typeof variants.primary] || variants.primary.neural;
  };
