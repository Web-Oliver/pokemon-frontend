...
export interface PokemonSelectProps extends Omit<React.SelectHTMLAttributes<HTMLSelectElement>, 'size'> {
  label?: string;
  error?: string;
  helper?: string;
  placeholder?: string;
...
export const PokemonSelect = forwardRef<HTMLSelectElement, PokemonSelectProps>(
  (
    ...
    const selectId = id || `pokemon-select-${Math.random().toString(36).substr(2, 9)}`;
