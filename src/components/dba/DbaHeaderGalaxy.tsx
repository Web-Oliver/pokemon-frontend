...
const DbaHeaderGalaxy: React.FC<DbaHeaderGalaxyProps> = ({
  ...
  const urgentCount = dbaSelections?.filter((s) => s.daysRemaining <= 10).length || 0;
      ...
      <div className="block lg:hidden">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-zinc-700 rounded-lg p-3 text-center">
            ...
            <p className="text-lg font-bold text-white">{dbaSelections?.length || 0}</p>
          </div>
          ...
          <div className="bg-green-900/30 rounded-lg p-3 text-center">
            ...
            <p className="text-lg font-bold text-white">{selectedItems.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-zinc-400">Export your collection items to DBA.dk</p>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <div className="bg-zinc-700 rounded-lg p-4 text-center min-w-[80px]">
            ...
            <p className="text-xs text-zinc-400">Queue</p>
            <p className="text-xl font-bold text-white">{dbaSelections?.length || 0}</p>
          </div>
          ...
          <div className="bg-green-900/30 rounded-lg p-4 text-center min-w-[80px]">
            ...
            <p className="text-xs text-green-400">Selected</p>
            <p className="text-xl font-bold text-white">{selectedItems.length}</p>
          </div>
        </div>
      </div>
    </div>
