...
export interface UseSearchSuggestionsReturn extends SuggestionsState {
  ...
  searchSuggestions: (query: string, type: 'set' | 'card' | 'product' | 'category') => Promise<void>;
  ...
  // Utility
  getSuggestionDisplayText: (suggestion: SetResult | CardResult | ProductResult | CategoryResult) => string;
}
...
export const useSearchSuggestions = (): UseSearchSuggestionsReturn => {
  ...
  const searchSuggestions = useCallback(async (query: string, type: 'set' | 'card' | 'product' | 'category') => {
    if (!query.trim()) {
      setState(prev => ({
        ...prev,
        suggestions: [],
