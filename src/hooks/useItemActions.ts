...
export const useItemActions = ({
  ...
  const getApiService = useCallback(() => {
    const collectionService = ServiceRegistry.getInstance().getCollectionApiService();
    ...
    switch (itemType) {
      case 'psa':
        return {
          ...
          getEditPath: (id: string) => `/collection/psa-graded-cards/${id}/edit`,
        };
      case 'raw':
        return {
  ...
  // Mark item as sold
  const handleMarkSold = useCallback(async (itemId: string, saleDetails: any) => {
    if (marking) return;
  ...
  const handleDownloadImages = useCallback(async (itemId: string) => {
    if (downloading) return;

    setDownloading(true);
    try {
      const exportService = ServiceRegistry.getInstance().getExportApiService();
...
export const usePsaItemActions = (options?: Omit<UseItemActionsOptions, 'itemType'>) =>
  useItemActions({ ...options, itemType: 'psa' });

export const useRawItemActions = (options?: Omit<UseItemActionsOptions, 'itemType'>) =>
  useItemActions({ ...options, itemType: 'raw' });

export const useSealedItemActions = (options?: Omit<UseItemActionsOptions, 'itemType'>) =>
