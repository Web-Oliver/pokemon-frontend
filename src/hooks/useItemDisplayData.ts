...
export const useItemDisplayData = ({ 
  ...
  return useMemo(() => {
    ...
    const formattedDate = item?.[dateField] ? formatDate(item[dateField]) : undefined;
    
    // Category information
    const categoryDisplayName = item?.itemCategory ? 
      formatItemCategory(item.itemCategory) : 
      (item?.category ? formatItemCategory(item.category) : 'Unknown');
    
    const categoryColorClass = item?.itemCategory ? 
      getItemCategoryColor(item.itemCategory) :
      (item?.category ? getItemCategoryColor(item.category) : '');
    
    // Status information (if requested)
    const statusColorClass = includeStatus && item?.status ? 
      getStatusColor(item.status) : undefined;
