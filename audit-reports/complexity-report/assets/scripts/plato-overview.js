...
$(function () {
  ...
  function drawFileCharts() {
    ...
    charts.each(function () {
      ...
      el.append(
        horizontalBar(value, Math.min(value * 2, width), 'complexity', [5, 10])
      );

      value = el.data('sloc');
      el.append(
        horizontalBar(value, Math.min(value, width), 'sloc', [400, 600])
      );

      value = el.data('bugs');
      el.append(
        horizontalBar(value, Math.min(value * 5, width), 'est errors', [1, 5])
      );
  ...
  function drawOverviewCharts(reports) {
    ...
    reports.forEach(function (report) {
      ...
      maintainability.data.push({
        value: report.complexity.maintainability
